...
def retrieve_all_confluence_documents():
    """
    Retrieve all direct child pages from the Confluence space home page.
    """
    try:
        ...
        # Get direct child pages of the home page
        children = confluence.get_child_pages(CONFLUENCE_HOME_PAGE_ID)
        logger.info(f"Found {len(children)} direct child pages in {CONFLUENCE_SPACE_KEY} space home page")
        all_documents = []
        for page in children:
            try:
                ...
                content_obj = confluence.get_page_by_id(page_id, expand='body.storage,version,history')
                content = content_obj['body']['storage']['value'] if 'body' in content_obj and 'storage' in content_obj['body'] else ''
                # Get metadata
                author = content_obj.get('history', {}).get('createdBy', {}).get('displayName', '')
                last_modified = content_obj.get('version', {}).get('when', '')
                doc = {
                    'document_id': page_id,
                    'title': title,
                    'url': url,
